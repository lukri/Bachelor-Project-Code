...
var VirtualContainer = function() {
	...
	this.addToObjList = function(newObj){
	        ...
	        lastActivatedObj = newObj;
	}
	
	this.toggleObject = function(objName){
		var objToActivate = objList[objName];
        if(lastActivatedObj){
            lastActivatedObj.visible=false;
            markerObject3D.remove(objToActivate);
        }
        markerObject3D.add(objToActivate);
