/*global awe, THREEx, Element, location */
//DEBUG = true;
...
function setGlobalVariables(){
    ...
    console.log(JSON.stringify(gloabalOptions));
...
var infoBox = null;
var info = null;
var marker, origin, board, ball, aim, crosshair, crosshairCenter, crosshairNeedle;
var fakeOriantation;
...
window.addEventListener('load', function() {
  ...
  infoBox = document.getElementById('markerDebugBox');
  infoBox.count = 0;
  info = document.getElementById('deviceDebugBox');
});
...
var startAWE = function(options){  
    ...
    Object.assign(gloabalOptions, options);



    if(gloabalOptions.fullscreen)
      toggleFullScreen();
...
function createAweScene(options){
  ...
  */
  ...
  origin = awe.pois.view('origin');
  origin.update({visible: false});
		        
  
  
  if(gloabalOptions.useNativ){
    awe.projections.add(
      { id:'board_projection', 
...
function doOnMarkerDetection(poi, marker_transform_matrix){
              ...
              origin.update({rotation: { x: 90, y:0, z:0}});
              origin.update({position: {x:position.x,y:position.y,z:-position.z}});
              if(!gloabalOptions.useNativ)
